...
//    this.numSkiers = Integer.parseInt(prop.getProperty("numSkiers"));
    ...
    client.setWriteTimeout(1000000);
    OkHttpClient httpClient = client.getHttpClient();
    httpClient.setConnectionPool(new com.squareup.okhttp.ConnectionPool(
        40, 100000)); //try 2000000
  ...
  public void run() {
    for (int post = 0; post < this.postReqNum; post++) {
      ...
      int statusCode = 500;
      try {
        this.apiInstance.writeNewLiftRide(newLift);
        ...
        System.out.println(newLift.toString());
    ...
    if(this.phase3) {
      for (int get = 0; get < this.getReqNum; get++) {
        ...
        try {
          ...
          statusCode = e.getCode();
          System.err.println("Exception when calling SkiersApi#getSkierResortTotalsWithHttpInfo");
          e.printStackTrace();
        }
        row.setLatency(String.valueOf(end - start));
        row.setResponseCode(String.valueOf(statusCode));
